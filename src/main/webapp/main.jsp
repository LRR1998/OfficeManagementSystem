...
<script type="text/javascript">
	    ...
	    $.ajax({
			...
			success:function (msg) {
				$(msg.map.list).each(function (index,item) {
					var li = "<li class='ue-clear'><span>"+moment(item.ndate).format('YYYY-MM-DD')+"</span>&nbsp;&nbsp;&nbsp;<a onclick='showDiv("+item.nid+")' href='javascript:void(0)' class='notice-title'>"+item.ntitle+"</a></li>"
					$("#noticeul").append(li);
				})
            }
        })
	...
	function showDiv(nid) {
	    $.ajax({
			...
			data:{"nid":nid},
			success:function (msg) {
                $("#ntitle").text(msg.ntitle).css({"font-weight":"bold","font-size":"25px"});
                $("#content").text(msg.remark).css("font-size","15px");

            }
		})
		$("#showdiv").show();
		$("#cover").css("height",document.body.scrollHeight+"px").show();
    }
...
<div id="showdiv" style="width: 60%; margin: 0 auto; height:500px; border: 1px solid #999; display: none; position: absolute; top: 20%; left: 20%; z-index: 3; background: #fff">
	...
	<!-- 内容 -->
	<div id="content" style="text-indent: 50px; height: 4rem; font-size: 0.5rem; padding: 0.5rem; line-height: 3rem; ">
	...
	<table width="98%" align="center" border="0" cellpadding="3"
		...
		<tr bgcolor="#FFFFFF">
			<td>
				<ul id="noticeul" class="notice-list">
