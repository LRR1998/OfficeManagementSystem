...
<tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='#FCFDEE';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22" >
	<td><input name="bxid" type="checkbox" id="id" value="${vs.bxid}" class="np"></td>
	<td>${bs.bxid}</td>
	<td>${bs.totalmoney}</td>
	...
	<c:if test="${bs.bxstatus == 0}">
		<td>未审批</td>
		<td><a href="baoxiao/edit?bxid=${bs.bxid}">编辑</a> </td>
	</c:if>
	<c:if test="${bs.bxstatus == 1}">
		<td>审批通过</td>
		<td><span href="baoxiao/detail?bxid=${bs.bxid}" style="pointer-events:none;color: grey" >查看</span> </td>
	</c:if>
	...
	<c:if test="${bs.bxstatus == 3}">
		<td>审批并付款</td>
		<td><a href="baoxiao/detail?bxid=${bs.bxid}" style="pointer-events:none;color: grey" >编辑</a> </td>
	</c:if>
...
<tr align="right" bgcolor="#EEF4EA">
	<td height="36" colspan="12" align="center">
		<a href="baoxiao/list?pageNum=1">首页</a>
		<c:if test="${page.isHasPreviousPage()}">
			<a href="baoxiao/list?pageNum=${page.prePage}">上一页</a>
		</c:if>
		<c:forEach items="${page.navigatepageNums}" var="num" >
			<a href="baoxiao/list?pageNum=${num}">${num}</a>
		</c:forEach>
		<c:if test="${page.isHasNextPage()}">
			<a href="baoxiao/list?pageNum=${page.nextPage}">下一页</a>
		</c:if>
		<a href="baoxiao/list?pageNum=${page.pageSize}">尾页</a>
	</td>
</tr>
</table>
